...
BlueMagicCameraController* BlueMagicCameraConnection::connect(BLEAddress add) 
{
  if (_cameraControl != nullptr)
  {
    return _cameraControl;
  ...
  bool scanned = scan(false, 5);
    ...
    ok = connectToServer(add);
  
  if (ok)
  {
    setAuthentication(true);
    _pref->begin(_name.c_str(), false);
    _pref->putString("cameraAddress", add.toString().c_str());
    _pref->putBool("authenticated", getAuthentication());
    _pref->end();
    setCameraAddress(add);
    return _cameraControl;
  }
